...
WITH LU_DimSeatStatus AS (
SELECT a.DimSeatStatusId, a.SeatStatusName

FROM (
	SELECT DimSeatStatusId, SeatStatusName
	, ROW_NUMBER() OVER(PARTITION BY SeatStatusName ORDER BY ETL__UpdatedDate) RowRank
	FROM etl.vw_DimSeatStatus (NOLOCK)
	WHERE ETL__SourceSystem = 'TM' AND DimSeatStatusId > 0 AND ETL__IsDeleted = 0
) a 
WHERE a.RowRank = 1
),
LU_DimSalesCodeName AS (
	...
	FROM (
		...
		WHERE ETL__SourceSystem = 'TM' AND DimSalesCodeId > 0 AND ETL__IsDeleted = 0
	) a 
	WHERE RowRank = 1
)
...
LEFT OUTER JOIN LU_DimSalesCodeName dSalesCodeName ON tkt.sell_location = dSalesCodeName.SalesCodeName
...
LEFT OUTER JOIN LU_DimSeatStatus dSeatStatus ON tkt.class_name = dSeatStatus.SeatStatusName
