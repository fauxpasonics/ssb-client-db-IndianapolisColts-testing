...
--select * from [etl].[vw_SFDCAccount_Load_DimCustomer]  where SSID = '0013100001hoaexAAA'
...
*/
    ...
    SELECT DB_NAME() SourceDB,
           ...
           NULL AS [Suffix],
                                                                  ...
                                                                  /*AddressPrimary*/
           [p].BillingStreet AS [AddressPrimaryStreet],
           [p].BillingCity AS [AddressPrimaryCity],
           [p].BillingState AS [AddressPrimaryState],
           [p].BillingPostalCode AS [AddressPrimaryZip],
           NULL AS [AddressPrimaryCounty],
           [p].[BillingCountry] AS [AddressPrimaryCountry],
           [p].[ShippingStreet] AS [AddressOneStreet],
           [p].[ShippingCity] AS [AddressOneCity],
           [p].[ShippingState] AS [AddressOneState],
           [p].[ShippingPostalCode] AS [AddressOneZip],
           NULL AS [AddressOneCounty],
           [p].[ShippingCountry] AS [AddressOneCountry],
           ...
           NULL AS [AddressFourSuite],
                                                                  /*Phone*/
           NULLIF(CAST(ISNULL(p.Phone, p.PersonMobilePhone) AS NVARCHAR(25)), '--') AS [PhonePrimary],
           NULL [PhoneHome],                                      -- CAST([p].[PersonHomePhone] AS NVARCHAR(25)) AS [PhoneHome]
           CAST(p.PersonMobilePhone AS NVARCHAR(25)) [PhoneCell], -- CAST([p].[PersonMobilePhone] AS NVARCHAR(25)) AS [PhoneCell]
           CAST(NULL AS NVARCHAR(25)) AS [PhoneBusiness],
           CAST(NULL AS NVARCHAR(25)) AS [PhoneFax],
           NULL [PhoneOther],                                     --  CAST([p].[PersonOtherPhone] AS NVARCHAR(25)) AS [PhoneOther]

                                                                  /*Email*/
           NULLIF(p.PersonEmail, '--') [EmailPrimary],            -- [p].[PersonEmail] AS [EmailPrimary]
           NULL AS [EmailOne],
           NULL AS [EmailTwo],
                                                                  /*Extended Attributes*/
           ...
           CASE
               ...
               ELSE
                   0
           END IsBusiness,
                                                                  --, CASE WHEN NULLIF(NULLIF(p.Archtics_ID_Winning_ID__c,''),'--') IS NULL THEN NULL ELSE CONCAT('TM - ',p.Archtics_ID_Winning_ID__c) END  AS customer_matchkey			
           NULL AS customer_matchkey
    FROM IndianapolisColts_Reporting.prodcopy.Account p
    WHERE 1 = 1
--AND 
--(p.CreatedDate > DATEADD(DAY,-3,GETDATE()) 
--OR p.LastModifiedDate > DATEADD(DAY,-3,GETDATE()) 
--OR p.CopyLoadDate >  DATEADD(DAY,-3,GETDATE()) 
--)
