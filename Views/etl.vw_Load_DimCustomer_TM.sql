...
CREATE VIEW [etl].[vw_Load_DimCustomer_TM]
AS(
SELECT *,
       /*Name*/
       HASHBYTES(
...
(
    ...
    SELECT DB_NAME() AS [SourceDB],
           ...
           CASE
               WHEN CAST(company_name AS NVARCHAR(50)) = '' THEN
                   NULL
               ELSE
                   CAST(company_name AS NVARCHAR(50))
           END AS [CompanyName],
           ...
           CONCAT(c.name_first, ' ', c.name_last) AS [FullName],
                                     ...
                                     /*AddressPrimary*/
           CASE
               WHEN ISNULL(street_addr_1, '') = ISNULL(street_addr_2, '') THEN
                   street_addr_1
               ELSE
                   street_addr_1 + ' ' + street_addr_2
           ...
           NULL AS [AddressFourSuite],
                                     /*Phone*/
           CASE
               WHEN LEFT(phone_day, 25) = '' THEN
                   NULL
               ELSE
                   LEFT(phone_day, 25)
           END AS [PhonePrimary],
           CASE
               WHEN LEFT(phone_eve, 25) = '' THEN
                   NULL
               ELSE
                   LEFT(phone_eve, 25)
           END AS [PhoneHome],
           CASE
               WHEN LEFT(phone_cell, 25) = '' THEN
                   NULL
               ELSE
                   LEFT(phone_cell, 25)
           END AS [PhoneCell],
        ...
        (
            ...
            (
                SELECT DISTINCT
                       ...
                       ca.name_prefix,
                       ca.name_suffix,
                       ROW_NUMBER() OVER (PARTITION BY ca.acct_id,
                                                       ca.cust_name_id
                                          ORDER BY ca.primary_ind DESC,
                                                   ca.UpdateDate DESC
                                         ) RowRank
